...
/**
    ...
    @title SYSG
    @brief Construye el precondicionador MILU.
*/
...
template<class P>
void MILU<P> :: calculate(P const &Mtx)
{
            ...
            while(lli < LU.irow[i+1] && llk < LU.irow[k+1]) 
            {
                if (ji == jk)
                {
                    if (ji > k && jk > k)
                    {
                        ...
                        LU.data[LU.idiag[i]] = LU.data[LU.idiag[i]]-L*LU.data[llk];
                    }
                    lli = lli + 1;
                    llk = llk + 1;
                    ji = LU.col[lli];
...
// Solve de MILU
template<class P>
void MILU<P> :: solve(Vector &x, Vector const &b)
{
